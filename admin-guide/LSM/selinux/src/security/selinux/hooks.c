			  ...
			  struct common_audit_data *adp)
{
	...
	/* 检查 inode 是否为私有（如 tmpfs 的匿名 inode），私有 inode 跳过 SELinux 检查 */
	if (unlikely(IS_PRIVATE(inode)))
		return 0;

	/* 获取当前任务的安全 ID (SID) */
	sid = cred_sid(cred);
	/* 获取 inode 的安全结构体，包含 inode 的 SID 和安全类别 */
	isec = selinux_inode(inode);

	/* 调用 AVC (Access Vector Cache) 检查权限：
	 * - sid: 任务的安全 ID
	 * - isec->sid: inode 的安全 ID
	 * - isec->sclass: inode 的安全类别（如文件、目录等）
	 * - perms: 请求的权限（如读、写、执行等）
	 * - adp: 可选的审计数据结构
	 */
...
/**
 * selinux_file_open - 文件打开时的 SELinux 安全检查
 * @file: 要打开的文件结构体
 *
 * 该函数在文件打开时执行 SELinux 安全检查。主要完成以下任务：
 * 1. 保存 inode 的安全标签（SID）和当前策略序列号，用于后续的权限重验证判断
 * 2. 执行实际的文件打开权限检查
 *
 * 注意：即使之前在 selinux_inode_permission 中已经检查过权限，这里仍然需要
 * 重新检查，因为 inode 标签或策略序列号可能在两次检查之间发生变化。
 *
 * 返回值: 0 表示成功，负值表示错误码
 */
static int selinux_file_open(struct file *file)
{
	struct file_security_struct *fsec;  /* 文件安全结构体指针 */
	struct inode_security_struct *isec; /* inode 安全结构体指针 */
	...
	/*
	 * 在打开时保存 inode 标签和策略序列号，以便 selinux_file_permission
	 * 能够判断是否需要重新验证权限。任务标签已经作为 SID 保存在文件安全
	 * 结构体中了。
	 */
	fsec->isid = isec->sid;           /* 保存 inode 的安全 ID */
	fsec->pseqno = avc_policy_seqno(); /* 保存当前策略序列号 */

	/*
	 * 由于在 selinux_inode_permission 检查和上述状态保存之间，inode 标签
	 * 或策略序列号可能已经发生变化，因此需要重新检查访问是否仍然被允许。
	 * 否则，可能永远不会针对新的 inode 标签或新策略重新验证访问权限。
	 * 这个检查不是多余的 - 请勿删除。
	 */
...
/**
 * selinux_init - SELinux 模块初始化函数
 *
 * 该函数在系统启动时由 LSM 框架调用，完成 SELinux 安全模块的核心初始化工作。
 * 初始化流程包括：状态结构清零、强制模式设置、AVC 缓存初始化、互斥锁初始化、
 * 初始任务安全上下文设置、内存执行策略检测、各类缓存初始化、LSM 钩子注册、
 * AVC 回调注册以及启动模式信息输出。
 *
 * 返回值: 始终返回 0 表示初始化成功（失败时会调用 panic）
 */
static __init int selinux_init(void)
{
	pr_info("SELinux:  Initializing.\n");

	/* 清零 selinux_state 全局状态结构体，确保初始状态干净 */
	...
	/* 初始化状态锁，用于保护 selinux_state 的并发访问 */
	mutex_init(&selinux_state.status_lock);

	/* 初始化策略锁，用于保护 SELinux 策略的加载和更新 */
	mutex_init(&selinux_state.policy_mutex);

	/* 为初始任务 (init_task) 设置安全上下文，标记为内核初始 SID */
	cred_init_security();

	/*
	 * 检测默认内存执行策略：
	 * VM_DATA_DEFAULT_FLAGS 定义了数据段的默认标志
	 * 如果未设置 VM_EXEC 标志，则默认不可执行，需要额外检查
	 */
	default_noexec = !(VM_DATA_DEFAULT_FLAGS & VM_EXEC);
	if (!default_noexec)
		pr_notice("SELinux:  virtual memory is executable by default\n");

	/* 初始化 AVC (Access Vector Cache) 机制，用于缓存权限决策 */
	avc_init();

	/* 初始化访问向量表 (AVTab) 缓存，用于策略规则存储 */
	avtab_cache_init();

	/* 初始化扩展位图 (ebitmap) 缓存，用于高效表示大型权限集合 */
	ebitmap_cache_init();

	/* 初始化哈希表缓存，用于策略中的各种查找操作 */
	hashtab_cache_init();

	/* 向 LSM 框架注册 SELinux 的所有安全钩子函数 */
	security_add_hooks(selinux_hooks, ARRAY_SIZE(selinux_hooks), "selinux");

	/*
	 * 注册网络缓存 AVC 回调函数：
	 * 当策略重置时，需要刷新网络相关的缓存（netif, netnode, netport）
	 */
	...
	/*
	 * 注册 LSM 通知器 AVC 回调函数：
	 * 当策略重置时，需要刷新 InfiniBand pkey 缓存并通知其他 LSM 模块
	 */
 ...
 * SELinux 需要尽早初始化，以便在进程和对象创建时对其进行标记。
 */
DEFINE_LSM(selinux) = {
	.name = "selinux", /* 模块名称 */
	...
	/*
	 * 模块标志：
	 * LSM_FLAG_LEGACY_MAJOR: 标识这是一个传统的“主”安全模块。
	 * LSM_FLAG_EXCLUSIVE: 标识这是一个独占性模块，意味着它不能与其他独占性 LSM（如 Smack、AppArmor）同时启用。
	 */
	.flags = LSM_FLAG_LEGACY_MAJOR | LSM_FLAG_EXCLUSIVE,

	/*
	 * 指向启用状态变量的指针。
	 * 指向 selinux_enabled_boot，该变量由启动参数 "selinux=" 控制。
	 */
	.enabled = &selinux_enabled_boot,

	/*
	 * 指向 LSM blob 大小定义的指针。
	 * 告知 LSM 框架 SELinux 需要在内核对象（如 cred, inode 等）中预留多少空间来存储安全上下文。
	 */
	.blobs = &selinux_blob_sizes,

	/*
	 * 初始化函数指针。
	 * LSM 框架在启动时会调用此函数来完成 SELinux 的核心初始化工作。
	 */
	.init = selinux_init,
};
