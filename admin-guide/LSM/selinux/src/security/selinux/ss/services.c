...
/**
 * avd_init - 初始化访问向量决策结构体
 * @policy: 当前 SELinux 策略指针，可为 NULL
 * @avd: 待初始化的访问向量决策结构体指针
 *
 * 该函数用于初始化 av_decision 结构体的各个字段：
 * - allowed: 初始化为 0，表示默认不允许任何权限
 * - auditallow: 初始化为 0，表示默认不审计允许的访问
 * - auditdeny: 初始化为 0xffffffff，表示默认审计所有拒绝的访问
 * - seqno: 如果 policy 有效，则使用策略的 latest_granting 序列号，
 *          否则设置为 0。该序列号用于 AVC 缓存失效检测
 * - flags: 初始化为 0，表示无特殊标志
 *
 * 注意：此函数在 RCU 读临界区内调用，policy 指针通过 RCU 保护
 */
 ...
 * @xperms: 扩展权限
 *
 * 根据 SID 对 (@ssid, @tsid) 计算针对 @tclass 中权限的访问向量决策集合。
 */
			 ...
			 struct extended_perms *xperms)
{
	...
	/* 根据 ssid 在 SID 表中查找源安全上下文 */
	scontext = sidtab_search(sidtab, ssid);
	if (!scontext) {
		/* 找不到源 SID，记录错误并跳转到清理流程 */
		pr_err("SELinux: %s:  unrecognized SID %d\n",
		       __func__, ssid);
		goto out;
	}
	...
	/*
	 * 检查源上下文类型是否在宽容域映射中。
	 * 如果是，设置 PERMISSIVE 标志，表示即使策略拒绝也实际允许访问（仅记录审计）。
	 */
	if (ebitmap_get_bit(&policydb->permissive_map, scontext->type))
		avd->flags |= AVD_FLAGS_PERMISSIVE;

	/* 根据 tsid 在 SID 表中查找目标安全上下文 */
	tcontext = sidtab_search(sidtab, tsid);
	if (!tcontext) {
		/* 找不到目标 SID，记录错误并跳转到清理流程 */
		pr_err("SELinux: %s:  unrecognized SID %d\n",
		       __func__, tsid);
		goto out;
	}
	...
	/*
	 * 将内核 API 使用的安全类别映射为策略内部定义的类别值。
	 * 内核定义的类名/值可能与策略文件中的定义不同。
	 */
	tclass = unmap_class(&policy->map, orig_tclass);

	/*
	 * 处理未知类别的情况：
	 * 如果传入的类别存在但映射后为空（策略中未定义），则根据策略配置处理。
	 * 如果策略允许未知类别，则放行；否则拒绝。
	 */
	if (unlikely(orig_tclass && !tclass)) {
		if (policydb->allow_unknown)
	...
	/*
	 * 核心计算函数：根据源上下文、目标上下文和安全类别，
	 * 遍历 TE 规则、约束、边界等，计算访问向量决策。
	 */
	context_struct_compute_av(policydb, scontext, tcontext, tclass, avd,
				  xperms);

	/*
	 * 将策略内部的权限位图映射回内核 API 定义的权限位图。
	 * 这同样是为了处理策略权限定义与内核定义不一致的情况。
	 */
...
/**
 * security_context_to_sid_core - 将安全上下文字符串转换为安全标识符 (SID)
 *
 * @scontext:  要转换的安全上下文字符串 (例如 "system_u:object_r:file_t:s0")
 * @scontext_len: 字符串的长度
 * @sid:       输出参数，用于存储转换后的 SID
 * @def_sid:   默认 SID，用于 MLS 字段解析时的默认值
 * @gfp_flags: 内存分配的 GFP 标志
 * @force:     强制标志。如果为 1，即使上下文无法在当前策略中解析
 *             （例如包含未知的用户/角色/类型），也会尝试将其作为
 *             未解释的原始字符串存储并分配一个 SID。
 *
 * 返回 0 表示成功，负值表示错误。
 */
					...
					int force)
{
	...
	/* 拷贝字符串以便后续修改，并确保以 NUL 结尾 */
	scontext2 = kmemdup_nul(scontext, scontext_len, gfp_flags);
	if (!scontext2)
		return -ENOMEM;

	/*
	 * 如果 SELinux 尚未初始化（通常在系统启动早期加载初始策略之前），
	 * 我们只能根据内置的初始 SID 列表进行简单的字符串匹配。
	 */
	if (!selinux_initialized()) {
		u32 i;
...
retry:
	...
	/*
	 * 如果解析失败（通常是因为策略中没有定义的组件）且处于强制模式，
	 * 则回退到将原始字符串作为上下文存储，不进行解释。
	 */
	if (rc == -EINVAL && force) {
		context.str = str;
		context.len = strlen(str) + 1;
		str = NULL; /* 所有权转移给 context，避免在 out 处重复释放 */
	} else if (rc) {
		/* 其他错误或非强制模式下的解析失败，直接退出 */
		goto out_unlock;
	}

	/* 将上下文结构体转换为 SID，可能会在 sidtab 中创建新条目 */
	rc = sidtab_context_to_sid(sidtab, &context, sid);

	/*
	 * 如果返回 -ESTALE，说明 SID 表正在进行转换（例如策略重载过程中），
	 * 当前操作无效，需要释放资源后重试。
	 */
	if (rc == -ESTALE) {
		rcu_read_unlock();
		/* 如果 context 中保存了原始字符串，需要将其归还给 str 以便下次重试 */
		if (context.str) {
			str = context.str;
			context.str = NULL;
		}
